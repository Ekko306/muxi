  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = () => {
      try {
        ...
        tt.verify(!!(res.stat || res.result), "networkError", "", res.data || res);
 ...
 * @param data
 */
export function post<T, U = any>(url: string, data?: any) {
  return request<T, U>({
    url,
    data,
    method: 'POST'
 ...
 * @param data
 */
export function get<T, U = any>(url: string, data?: any) {
  return request<T, U>({
    url,
    data,
    method: 'GET'
